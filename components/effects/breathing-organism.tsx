 ...
 * Pure SVG, no WebGL = lightweight and crisp at any DPR.
 *
 * Visually: 4 overlapping radial gradient blobs animated with offset phases,
 * gives the impression of a living, slow-pulsing organism behind the hero.
 */
export function BreathingOrganism() {
  ...
  // Convert 0..1 to small CSS percentage offset
  ...
  return (
    ...
    >
      ...
      <div
        ...
        style={{
          background: "linear-gradient(180deg, #f3ede2 0%, #faf6ed 60%, #f0e9d9 100%)",
        }}
      />
      {/* breathing blob 1 — olive, slow */}
      <motion.div
        style={{ x: tx1, y: ty1 }}
        animate={{ scale: [1, 1.08, 1] }}
        transition={{ duration: 5, repeat: Infinity, ease: "easeInOut" }}
        className="absolute -top-[20%] -left-[15%] h-[70vw] w-[70vw] rounded-full opacity-60 blur-3xl"
        style={{ background: "radial-gradient(closest-side, #c2cc9a, transparent 70%)" }}
      />
      {/* breathing blob 2 — saffron, slower */}
      <motion.div
        style={{ x: tx2, y: ty2 }}
        animate={{ scale: [1, 1.12, 1] }}
        transition={{ duration: 7, repeat: Infinity, ease: "easeInOut", delay: 1.2 }}
        className="absolute -bottom-[25%] -right-[10%] h-[60vw] w-[60vw] rounded-full opacity-55 blur-3xl"
        style={{ background: "radial-gradient(closest-side, #e8c692, transparent 70%)" }}
      />
      {/* breathing blob 3 — cream, micro pulse */}
      <motion.div
        style={{ x: tx3, y: ty3 }}
      ...
      />
      {/* deep moss anchor at bottom for contrast */}
      <div
